...
export function getMinimumAllowedBirthDate(minAge = 16) {
  const date = new Date();
...
export function isAtLeastAge(birthDate, minAge = 16) {
  if (!birthDate) {
    return false;
  }

  return String(birthDate) <= getMinimumAllowedBirthDate(minAge);
}
