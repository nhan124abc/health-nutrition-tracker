...
function DailyMealSummary({ calorieGoal = 2000, mealCount, t, totals }) {
  ...
  const status = percent > 100
    ? { variant: 'danger', text: t('foodDiaryPage.summary.over', { calories: totals.calories - calorieGoal }) }
    : percent >= 90
      ? { variant: 'warning', text: t('foodDiaryPage.summary.nearLimit') }
      : percent < 60
        ? { variant: 'info', text: t('foodDiaryPage.summary.remaining', { calories: calorieGoal - totals.calories }) }
  ...
  return (
    <Card className="border-0 shadow-sm planner-side-card sticky-panel">
      <Card.Body>
        ...
        <div className="d-flex justify-content-between small mb-1"><span>{t('foodDiaryPage.summary.progress')}</span><strong>{totals.calories} / {calorieGoal} kcal</strong></div>
