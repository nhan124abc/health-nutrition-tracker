    ...
    <Card className="border-0 shadow-sm planner-side-card sticky-panel">
      <Card.Body>
        ...
        <div className="nutrition-detail-grid">
          <div><span>{t('common.protein')}</span><strong>{formatMacro(totals.protein)}g</strong></div>
          <div><span>{t('common.carbs')}</span><strong>{formatMacro(totals.carbs)}g</strong></div>
          <div><span>{t('common.fat')}</span><strong>{formatMacro(totals.fat)}g</strong></div>
          <div><span>{t('common.fiber')}</span><strong>{formatMacro(totals.fiber)}g</strong></div>
          <div><span>{t('common.sodium')}</span><strong>{formatMacro(totals.sodium, 0)}mg</strong></div>
        </div>
      </Card.Body>
    </Card>
  );
